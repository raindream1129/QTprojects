    ...
    ui(new Ui::Widget)
{
    ...
    connect(Timer_A, SIGNAL(timeout()),
            this, SLOT(TTimeout_A()));
    ...
    ui->AppSevIPLE->setValidator(ipRegExpValidator);
    ui->AppSevIPLE->setInputMask("000.000.000.000; ");

    ui->BackUpIPLE->setValidator(ipRegExpValidator);
    ui->BackUpIPLE->setInputMask("000.000.000.000; ");
}
...
//            ui->SYSState->setText(ST_Meter);
    ...
    */
    ...
    //读固件版本号
    case TASK_READ_FIRMWARE_VERSION:
    {
        if(cjt188Recv.ctrlId == CJT188_FIO_CTRL_ID_RD_DATA_SUCCESS)
        {
            QString sV;
            BigToLittle(cjt188Recv.value,&sV);
            qDebug()<<cjt188Recv.value;
            ui->FIRMWARE_VERSION_LE->setText(cjt188Recv.value);
            WriteLog.Result = "success";
            logHandler("读固件版本信息成功");
        }
        else{
            WriteLog.Result = "fail";
            logHandler("读固件版本信息失败");
        }
        break;
    }
    //读温度
...
//            QString sICCID = cjt188Recv.value.mid(32,40);
    ...
    case TASK_READ_TOTALIZER_STYLE:
    {
        //读累加器类型
        if(cjt188Recv.ctrlId == CJT188_FIO_CTRL_ID_RD_DATA_SUCCESS)
        {
            if(cjt188Recv.value == "00")
            {
                ui->TOTALIZER_STYLE_CB->setCurrentIndex(0);
            }
            else if(cjt188Recv.value == "01")
            {
                ui->TOTALIZER_STYLE_CB->setCurrentIndex(1);
            }
            else{
                QMessageBox::warning(this,"提示","未知累加器类型");
                break;
            }
            WriteLog.Result = "success";
            logHandler("读累加器类型成功");
    ...
    case TASK_WRITE_TOTALIZER_STYLE:
    {
        //写累加器类型
        if(cjt188Recv.ctrlId == CJT188_FIO_CTRL_ID_WR_DATA_SUCCESS)
        {
...
void Widget::TTimeout_A()
{
    ...
    case 18:
    {
        ...
        meterRoutine.errCount = 0;
        meterRoutine.rsp = METER_RSP_NONE;
        meterRoutine.task = TASK_READ_FIRMWARE_VERSION;
        cjt188obj.ctrlId = CJT188_FIO_CTRL_ID_RD_DATA;
        cjt188obj.dataId = CJT188_FIO_OBJ_ID_UPDATE_NEW_VERSION;
        WriteLog.desc = "get the gas meter's firmware version";
        BuiltLogFileExe();
        break;
    }
    case 19:
    {
        ...
        meterRoutine.errCount = 0;
        meterRoutine.rsp = METER_RSP_NONE;
        meterRoutine.task = TASK_READ_TOTALIZER_STYLE;
        cjt188obj.ctrlId = CJT188_FIO_CTRL_ID_RD_DATA;
        cjt188obj.dataId = CJT188_FIO_OBJ_ID_TOTALIZER_STYLE;
        WriteLog.desc = "get the gas meter's totalizer's style";
        BuiltLogFileExe();
...
void Widget::on_AlarmFlow_BTN_clicked()
{
    ...
    meterRoutine.errCount = 0;
    meterRoutine.rsp = METER_RSP_NONE;
    meterRoutine.task = TASK_WRITE_CHARGE;
 ...
 * 计量数据
 * ****************************************/
void Widget::on_RD_MTRLGC_DATA_BTN_clicked()
{
    cjt188obj.value.clear();
    WriteLog.clear();
    ui->Device_timeLE->clear();
 ...
 * 外电电压
 * ***********************************/
void Widget::on_RD_EXTERNAL_V_clicked()
{
    cjt188obj.value.clear();
    ui->lineEV->clear();
    meterRoutine.errCount = 0;
 ...
 * 设备运行时间
 * ***************************************/
void Widget::on_RD_RUNNIN_TIME_BTN_clicked()
{
    cjt188obj.value.clear();
    ui->WorkingT_LE->clear();
    meterRoutine.errCount = 0;
 ...
 * 版本号
 * ***************************************/
void Widget::on_RD_FIRMWARE_VERSION_BTN_clicked()
{
    cjt188obj.value.clear();
    ui->SEV_Version_LE->clear();
    meterRoutine.errCount = 0;
 ...
 * 温度
 * ***************************************/
void Widget::on_RD_TEMP_BTN_clicked()
{
    cjt188obj.value.clear();
    ui->Temp_LE->clear();
    meterRoutine.errCount = 0;
 ...
 * 连续掉线时间
 * ***************************************/
void Widget::on_RD_OFFLINE_TIME_BTN_clicked()
{
    cjt188obj.value.clear();
    ui->DropTimeLE->clear();
    meterRoutine.errCount = 0;
 ...
 * 透支额度
 * ***************************************/
void Widget::on_RD_OVERDRAFT_BTN_clicked()
{
    cjt188obj.value.clear();
    ui->lineEdit_2->clear();
    meterRoutine.errCount = 0;
 ...
 * 固件版本号
 * ***************************************/
void Widget::on_FIRMWARE_VERSION_BTN_clicked()
{
    cjt188obj.value.clear();
    ui->FIRMWARE_VERSION_LE->clear();
    meterRoutine.errCount = 0;
 ...
 * 模式
 * ***************************************/
void Widget::on_RD_MODEL_BTN_clicked()
{
    cjt188obj.value.clear();
    meterRoutine.errCount = 0;
    meterRoutine.rsp = METER_RSP_NONE;
...
//电压阈值
void Widget::on_RD_THRESHOLD_BTN_clicked()
{
    ui->lineEVTh->clear();
    cjt188obj.value.clear();
    meterRoutine.errCount = 0;
...
//最大流量
void Widget::on_RD_MAX_FLOW_BTN_clicked()
{
    cjt188obj.value.clear();
    ui->MAX_FLOW_LE->clear();
    meterRoutine.errCount = 0;
...
//最小流量
void Widget::on_RD_MIN_FLOW_BTN_clicked()
{
    ui->Min_Flow_LE->clear();
    cjt188obj.value.clear();
    meterRoutine.errCount = 0;
...
//已闲置时长
void Widget::on_RD_IDLE_TIME_BTN_clicked()
{
    cjt188obj.value.clear();
    ui->IdledTs_LE->clear();
    meterRoutine.errCount = 0;
...
//闲置超时
void Widget::on_RD_ILDE_OVERTIME_BTN_clicked()
{
    cjt188obj.value.clear();
    ui->IdleT_LE->clear();
    meterRoutine.errCount = 0;
...
//余额
void Widget::on_RD_BALANCE_BTN_clicked()
{
    cjt188obj.value.clear();
    ui->Alarm_Bal_LE->clear();
    meterRoutine.errCount = 0;
...
//余量
void Widget::on_RD_REMAINGAS_BTN_clicked()
{
    cjt188obj.value.clear();
    ui->Alarm_Gas_LE->clear();
    meterRoutine.errCount = 0;
...
//警示额度
void Widget::on_RD_ALARM_BALANCE_BTN_clicked()
{
    cjt188obj.value.clear();
    ui->lineEdit->clear();
    meterRoutine.errCount = 0;
...
//系统状态
void Widget::on_SYS_STATUS_BTN_clicked()
{
    cjt188obj.value.clear();
    ui->SYS_ST_LE->clear();
    meterRoutine.errCount = 0;
...
void Widget::on_pushButton_7_clicked()
{
    ...
    meterRoutine.errCount = 0;
    meterRoutine.rsp = METER_RSP_NONE;
    meterRoutine.task = TASK_WRITE_TOTALIZER_STYLE;
    cjt188obj.ctrlId = CJT188_FIO_CTRL_ID_WR_DATA;
    cjt188obj.dataId = CJT188_FIO_OBJ_ID_TOTALIZER_STYLE;
    qDebug()<<ui->TOTALIZER_STYLE_CB->currentIndex();
    if(ui->TOTALIZER_STYLE_CB->currentIndex() == 0)
    {
        cjt188obj.value = "00";
    }
    else if(ui->TOTALIZER_STYLE_CB->currentIndex() == 1)
    {
        cjt188obj.value = "01";
    }
    else{
    ...
    WriteLog.desc = "set the gas meter's totalizer's style";
    BuiltLogFileExe();
